...
int part1(String header, List input) {
  ...
  var writeInput = 1;
  var returnAddress = 0;
  return Computer(input).run(returnAddress, [writeInput]).first;
}

int part2(String header, List input, int initValue) {
  printHeader(header);
  // print(input);
  // var writeInput = 5;
  var returnAddress = 0;
  return Computer(input).run(returnAddress, [initValue]).first;
}
